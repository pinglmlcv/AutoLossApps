'''
The module for training machine translation task.
...
class Trainer():
    ...
    def train(self, load_model=None, save_model=False):
        ...
        for step in range(config.max_training_steps):
            ...
            step_loss, summary = model.train(inputs, inputs_len,
                                             targets, targets_len)
            ...
            if real_step % config.display_frequency == 0:
                ...
                logger.info('Step: {}, Perplexity: {}, Loss: {}'.\
                            format(real_step, avg_perplexity, loss))
                loss = 0
                start_time = time.time()
            ...
            if real_step % config.valid_frequency == 0:
                ...
                logger.info('Valid preplexity: {}, Loss: {}'.\
                            format(math.exp(valid_loss), valid_loss))
    ...
    def valid(self):
        ...
        while dataset.epochs_completed < 1:
            ...
            inputs, inputs_len, targets, targets_len = prepare_train_batch(
                inputs, targets, config.max_seq_length)
            batch_loss, summary = self.model.eval(inputs, inputs_len,
                                                  targets, targets_len)
...
if __name__ == '__main__':
    ...
    cfg_dir = os.path.join(root_path, 'config/mtRNNAttention/')
