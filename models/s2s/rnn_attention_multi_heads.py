...
class RNNAttentionMultiHeads(Basic_model):
    def __init__(self, config, sess, mode, exp_name='RNNAttentionMultiHeads', logger=None):
        self.mode = mode.lower()

        # Config must define RNN type and attention type
        super(RNNAttentionMultiHeads, self).__init__(config, sess, exp_name)
        self.logger = logger
    ...
    def _build_decoder(self, task):
        ...
        if not logger:
            ...
            print('building decoder and attention for {} task...'.format(task))
    ...
    def init_optimizer(self, task):
        ...
        if config.optimizer.lower() == 'adadelta':
            opt = tf.train.AdadeltaOptimizer(learning_rate=config.learning_rate)
        elif config.optimizer.lower() == 'adam':
            opt = tf.train.AdamOptimizer(learning_rate=config.learning_rate)
        elif config.optimizer.lower() == 'rmsprop':
            opt = tf.train.RMSPropOptimizer(learning_rate=config.learning_rate)
        else:
    ...
    def train(self, task_num, encoder_inputs, encoder_inputs_length,
