...
class AgentGridWorld(Basic_model):
    ...
    def _build_graph(self):
        ...
        self.q_learning_loss = q_learning_loss
    ...
    def update(self, transition_batch, lr=None, mode=None):
        if not mode:
            if self.hybrid:
                return self.update_distill(transition_batch, lr)
            else:
                return self.update_q_learning(transition_batch, lr)
        elif mode.lower() == 'distill':
            return self.update_distill(trainsition_batch, lr)
    ...
    def update_distill(self, transition_batch, lr=None):
        ...
        fetch = [self.distill_op, self.distill_loss]
        feed_dict = {self.state: state,
                     ...
                     self.lr: lr}
        _, distill_loss = self.sess.run(fetch, feed_dict)
        #logger.info('distill_loss: {}'.format(distill_loss))
        return distill_loss
    ...
    def update_q_learning(self, transition_batch, lr=None):
        ...
        next_state = transition_batch['next_state']
        q_expert = transition_batch['q_expert']
        fetch = [self.q_learning_op, self.q_learning_loss]
        feed_dict = {self.state: state,
                     self.next_state: next_state,
                     self.reward: reward,
                     self.action: action,
                     self.q_expert: q_expert,
                     self.lr: lr}
        _, q_learning_loss = self.sess.run(fetch, feed_dict)
        #logger.info('q_learning_loss: {}'.format(q_learning_loss))
        return q_learning_loss

    def run_step(self, state, greedy=True, epsilon=0):
        config = self.config.agent
        dim_a = config.dim_a
        ...
        if greedy:
            ...
            p = np.exp(q_values) / np.exp(q_values).sum()
            A = p
